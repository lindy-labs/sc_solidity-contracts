...
| --------          | -------- |    
| Description:      | When user withdraw the principal from the vault, the vault may leave some dust due to rounding of uint division in Solidity. E.g., user withdraws 1000 LUSD, the vault may send only 999.99999... LUSD if the share price is not exactly an integer, which can happen when underlying asset value has changed, e.g., yield harvested | 
| Mitigation/Fix:   | update the `_withdrawSingle` internal function to remove the redundant and imprecise `computeAmount` function call and return the `_amount` value user requests |
| Property violated:| When user withdraw principal, the requested amount should be sent to the user|
