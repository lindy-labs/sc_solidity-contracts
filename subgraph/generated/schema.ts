...
export class Vault extends Entity {
  ...
  get dcas(): Array<string> {
    let value = this.get("dcas");
    return value!.toStringArray();
  }

  set dcas(value: Array<string>) {
    this.set("dcas", Value.fromStringArray(value));
  }
...
export class DCA extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));

    this.set("vault", Value.fromString(""));
    this.set("input", Value.fromBytes(Bytes.empty()));
    this.set("output", Value.fromBytes(Bytes.empty()));
  }

  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save DCA entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save DCA entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("DCA", id.toString(), this);
    }
  }

  static load(id: string): DCA | null {
    return changetype<DCA | null>(store.get("DCA", id));
  }
