...
contract TestCurveSwapper is CurveSwapper {
    ...
    function test_swapIntoUnderlying(address _token, uint256 _amount, uint256 slippage) external {
        _swapIntoUnderlying(_token, _amount, slippage);
    }

    function test_swapFromUnderlying(address _token, uint256 _amount, uint256 slippage) external {
        _swapFromUnderlying(_token, _amount, slippage);
    }
