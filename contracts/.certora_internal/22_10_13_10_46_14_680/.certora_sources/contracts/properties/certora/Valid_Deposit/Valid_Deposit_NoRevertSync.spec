...
rule VaultBalanceIncreases {
    ...
    uint256 balance_vault_before;
    require balance_vault_before == vault.totalUnderlying();

    //depositParts(eV, inputToken, lockDuration, amount, name, slippage);

    uint256 balance_vault_after;
    require balance_vault_after == vault.totalUnderlying();
