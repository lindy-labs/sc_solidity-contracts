...
/*    function depositParts(uint64 lockDuration, uint64 amount, string name, uint256 slippage) {
        DepositParams _params;

        require _params.inputToken == getAdd();//address(underlying);
        require _params.lockDuration == 2 * 7 + (lockDuration % (22 * 7));//lockDuration; name weeks is not defined in Certora
        require _params.amount == amount;
        require _params.name == name;
        require _params.slippage == slippage;
...
rule VaultBalanceIncreases {
    ...
    //mint_helper(eV, eV.msg.sender, amount);
        underlying.mint(eV.msg.sender, amount);
        underlying.approve(currentContract, amount);//address(vault)

    uint256 balance_vault_before = /*vault.*/totalUnderlying();

        //depositIds = vault.deposit(_params);
        deposit(eV, getParams(eV, lockDuration, amount, name, slippage));

    //depositParts(eV, lockDuration, amount, name, slippage);
